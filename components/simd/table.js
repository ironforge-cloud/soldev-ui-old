      ...
      <div className="mt-8 flex flex-col">
        <div className="-my-2 -mx-4 overflow-x-auto sm:-mx-6 lg:-mx-8">
          <div className="inline-block min-w-full py-2 align-middle md:px-6 lg:px-8">
            <div className="overflow-hidden rounded-lg border-[1px] border-gray-500 shadow ring-1 ring-transparent ring-opacity-5 dark:border-gray-300">
              <table className="min-w-full divide-y divide-gray-500 dark:divide-gray-300">
                <thead className="bg-gray-100 text-center">
                  <tr className="divide-x divide-gray-500 dark:divide-gray-300">
                ...
                <tbody className="divide-y divide-gray-500 dark:divide-gray-300">
                  {sortedContent.map((item, idx) => (
                    <tr
                      key={idx}
                      className={classNames(
                        'divide-x divide-gray-500 whitespace-nowrap text-center font-semibold text-gray-500 dark:divide-gray-300 dark:text-gray-400'
                      )}
                    >
                      <td className="px-3 py-4 text-sm">
                        {item?.metadata?.simd ? item?.metadata?.simd : '-'}
                      </td>
                      <td
                        className="cursor-pointer px-3 py-4 text-sm no-underline hover:text-[#161614] hover:underline dark:hover:text-gray-200"
                        title={item?.metadata?.title}
                      >
                        <Link href={`/simd/${item?.metadata?.simd}`}>
                      ...
                      <td className="px-3 py-4">
                        {item?.metadata?.status ? (
                          <span className="inline-flex rounded-full border-[1px] border-gray-700 bg-green-100 px-2 text-xs leading-5 text-green-800  dark:border-none dark:bg-orange-100 dark:text-orange-800">
                            {item?.metadata?.status}
                          </span>
                        ) : (
                          '-'
                      ...
                      <td className="px-3 py-4 text-sm">
                        {item.metadata.authors ? (
                          item?.metadata?.authors.map((author, idx) => (
                            <div key={idx}>
                              {truncateText(author.name, 30)} {author.org && `(${author.org})`}
                            </div>
                          ))
                        ) : (
                          <p>-</p>
                      ...
                      <td className="px-3 py-4 text-sm">
                        {item?.metadata?.created ? item?.metadata?.created : '-'}
                      </td>
                      <td className="px-3 py-4 text-sm">
                        <a
                          href={item?.html_url}
