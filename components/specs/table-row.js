...
import CheckMark from '../course/check-mark';
...
function TableRow({ item, index, ready }) {
  ...
  return (
    ...
    >
      <Link href={`specs/${title}`}>
        <span className="mr-3 text-xl text-gray-400 dark:text-gray-500">{++index}.</span>
        <span className=" tracking-wide text-gray-700 dark:text-gray-300">{title}</span>
      </Link>

      <CheckMark item={item.download_url} />
    </div>
  );
}
